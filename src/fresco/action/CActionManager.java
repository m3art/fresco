...
imageWorker = null;
         ...
         private class MIPUtils {
            ...
            public void refillParams() {
                for (int j = 0; j < paramCount; j++) {
                    ...
                    if (growth[j] > 0) growth[j] -= lambda*pc[j].getRegularization();
                    else growth[j] += lambda*pc[j].getRegularization();
                }
            ...
            public void runIntPtsOnAllImgs() {
                ...
                if (CData.imagesSize() == i+1) break;
                    ...
                    for (int j = 0; j < paramCount+1; j++) {
                      ...
                      workerFieldCOG[j] = new CInterestingPoints(CData.getImage(CData.showImage[0]).getImage(), CData.getImage(CData.showImage[2]).getImage(), Cornerer.COG, Edger.sobel, pc[j], pe[j], j, i);
                      threadFieldCOG[j] = new CInterestingPointsThread(workerFieldCOG[j]);
  ...
  public void runMultipleIntPoints() throws InterruptedException {
      ...
      for (int i = 0; i < setSize; i++) {
        ...
        CInterestingPoints validWorker = new CInterestingPoints(CData.getImage(CData.showImage[0]).getImage(), CData.getImage(CData.showImage[2]).getImage(), Cornerer.COG, Edger.sobel, m.pc[0], m.pe[0], -1, 2*heldout);
        CInterestingPointsThread validThread = new CInterestingPointsThread(validWorker);
        validThread.start();
        validThread.join();
        
        CInterestingPoints randWorker = new CInterestingPoints(CData.getImage(CData.showImage[0]).getImage(), CData.getImage(CData.showImage[2]).getImage(), Cornerer.random, Edger.sobel, m.pc[0], m.pe[0], -2, 2*heldout);
        CInterestingPointsThread randThread = new CInterestingPointsThread(randWorker);
        randThread.start();
        randThread.join();
