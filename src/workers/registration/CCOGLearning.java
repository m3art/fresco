 ...
 * @author Jakub
 */
public final class CCOGLearning {
	...
	/**
	 * learning rate for the variance of the LoG edge detector gaussian one
	 * order of magnitude greater than COG params, as values are also an order
	 * of magnitude larger
	 */
	...
	/**
	 * growth of the score function in the direction of the params, as
	...
	public void calculateWindowSizes() {
		for (int i = 0; i < CData.imagesSize(); i += 2) {
			...
			windowSizes[i / 2] = 11;
			logger.log(Level.INFO, "windowsize: {0}", windowSizes[i / 2]);
		}
		imageA = null;
	 ...
	 * parameters by derStep setting them up for threads to calculate
	 * derivatives in directions of the params
	 */
...
//        show2ndInput();
			...
			for (int j = 0; j < paramCount + 1; j++) {
				try {
					...
					logger.log(Level.SEVERE, "Thread {0} interrupted", j);
				}
	...
	private void runLearning(int[] imgList) {
		for (int iter = 0; iter < maxIters; iter++) {
			logger.log(Level.INFO, "started iter {0}", iter);
			initParams();
	...
	public void runCrossVal() throws InterruptedException {
		...
		for (int i = 0; i < setSize; i++) {
			...
			// why you are running this in separate thread? Paralelism? This way?
